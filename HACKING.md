...
CMake's `FetchContent`, so no external package manager is required. A working
internet connection is needed the first time you configure the project.
...
Runs the executable target `parser_exe`.
